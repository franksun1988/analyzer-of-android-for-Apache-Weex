...
public abstract class BaseSeries<E extends DataPointInterface> implements Series<E> {
     ...
     */
    private List<ChartView> mGraphViews;

    /**
     * creates series without data
     */
    public BaseSeries() {
        mGraphViews = new ArrayList<ChartView>();
    }
    ...
    /**
     * getPerformanceList the values for a given x range. if from and until are bigger or equal than
     * all the data, the original data is returned.
     * If it is only a part of the data, the range is returned plus one datapoint
     * before and after to getPerformanceList a nice scrolling.
     *
     ...
     */
    @Override
    public void onGraphViewAttached(ChartView graphView) {
